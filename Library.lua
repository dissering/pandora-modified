...
local Library do 
    ...
    local Instances = { } do
        ...
        -- Keep the library skinnable without requiring every control to manually
        -- register each of its colours.  These are the colours used by the
        -- original Pandora palette, mapped to their semantic theme slot.
        local LegacyThemeColors = {
            ...
            ["221,221,221"] = "Text",
            ["166,147,243"] = "Accent",
            ["110,97,161"] = "Dark Accent",
            ["255,240,123"] = "Accent"
        }

        local GetThemeSlot = function(Value)
            if typeof(Value) ~= "Color3" then
                return nil
            end

            local Key = StringFormat("%d,%d,%d", MathFloor(Value.R * 255 + 0.5), MathFloor(Value.G * 255 + 0.5), MathFloor(Value.B * 255 + 0.5))
            return LegacyThemeColors[Key]
        end
        ...
        Instances.Create = function(self, Class, Properties)
            ...
            for Property, Value in NewItem.Properties do
                local ThemeSlot = GetThemeSlot(Value)

                if ThemeSlot then
                    NewItem.Instance[Property] = Library.Theme[ThemeSlot]
                    ThemeProperties[Property] = ThemeSlot
                else
            ...
            if next(ThemeProperties) then
                local ThemeData = {
                    Item = NewItem.Instance,
                    Properties = ThemeProperties,
                }

                TableInsert(Library.ThemeItems, ThemeData)
                Library.ThemeMap[NewItem.Instance] = ThemeData
            end
    ...
    Library.AddToTheme = function(self, Item, Properties)
        ...
        local ExistingThemeData = self.ThemeMap[Item]
        if ExistingThemeData then
            for Property, Value in Properties do
                ExistingThemeData.Properties[Property] = Value
            end
        ...
        local Success, Result = Library:SafeCall(function()
            for Index, Value in Library.Flags do 
                if type(Value) == "table" and Value.Key then
                    Config.Flags[Index] = {Key = tostring(Value.Key), Mode = Value.Mode}
                elseif type(Value) == "table" and Value.Color then
                    Config.Flags[Index] = {Color = "#" .. Value.HexValue, Alpha = Value.Alpha}
                else
                    Config.Flags[Index] = Value
                end
            end

            for Index, Value in Library.Theme do
                if typeof(Value) == "Color3" then
                    Config.Theme[Index] = "#" .. Value:ToHex()
                end
            end
        end)
    ...
    Library.ApplyConfigFlag = function(self, Flag)
        if not self.PendingConfig then
            return
        end

        local Value = self.PendingConfig[Flag]
        local SetFunction = self.SetFlags[Flag]

        if Value == nil or not SetFunction then
            return
        end

        local WasLoading = self.IsLoadingConfig
        ...
        self.IsLoadingConfig = WasLoading
        self.PendingConfig[Flag] = nil
    end

    Library.RegisterFlag = function(self, Flag, SetFunction)
        self.SetFlags[Flag] = SetFunction
        self:ApplyConfigFlag(Flag)
    end
    ...
    Library.LoadConfig = function(self, Config)
        ...
        if not Success or type(Decoded) ~= "table" then
            return false, Result or "Invalid settings file"
        end
        ...
        for Index, Value in self.PendingConfig do
            self:ApplyConfigFlag(Index)
        end
    ...
    Library.SaveAutoConfig = function(self)
        if not self.AutoSaveEnabled then
            return false, "Automatic saving is not enabled yet"
        end

        return self:SafeCall(function()
            writefile(self.ConfigFile, self:GetConfig())
        end)
    end

    Library.QueueAutoSave = function(self)
        if not self.AutoSaveEnabled or self.IsLoadingConfig or self.AutoSaveQueued then
            return
        end
        ...
        task.delay(0.4, function()
            ...
            if self.AutoSaveEnabled and not self.IsLoadingConfig then
                self:SaveAutoConfig()
            end
        end)
    end

    Library.LoadAutoConfig = function(self)
        if not isfile(self.ConfigFile) then
            return false, "No saved settings"
        end

        return self:LoadConfig(readfile(self.ConfigFile))
    end
    ...
    do 
        Library.CreateColorpicker = function(self, Data)
            ...
            function Colorpicker:Update(IsFromAlpha)
                ...
                Library:SetFlag(Colorpicker.Flag, {
        ...
        Library.Window = function(self, Data)
            ...
            local Window = {
                ...
                Title = Data.Title or Data.title or "Window",
            ...
            local Items = { } do
                Items["MainFrame"] = Instances:Create("Frame", {
                    ...
                    BorderColor3 = FromRGB(0, 0, 0),
                    Size = UDim2New(0, 640, 0, 540),
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(20, 20, 20)
                })  Items["MainFrame"]:AddToTheme({BackgroundColor3 = "Background"})
                
                Items["MainFrame"]:MakeDraggable()
                Items["MainFrame"]:MakeResizeable(Vector2New(640, 540), Vector2New(9999, 9999))
                ...
                Items["Top"] = Instances:Create("Frame", {
                    ...
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(26, 26, 26)
                })  Items["Top"]:AddToTheme({BackgroundColor3 = "Surface"})
                ...
                Items["SubPages"] = Instances:Create("Frame", {
                    ...
                    BackgroundColor3 = FromRGB(26, 26, 26)
                })  Items["SubPages"]:AddToTheme({BackgroundColor3 = "Surface"})
        ...
        Library.Page = function(self, Data)
            ...
            local Items = { } do
                Items["Inactive"] = Instances:Create("TextButton", {
                    ...
                    TextSize = 14,
                    BackgroundColor3 = FromRGB(255, 255, 255)
                })  Items["Inactive"]:AddToTheme({TextColor3 = "Muted"})
                ...
                Items["Inactive"]:OnHoverLeave(function()
                    if Page.Active then return end
                    Items["Inactive"]:Tween(nil, {TextColor3 = Library.Theme.Muted})
                end)
            ...
            function Page:Turn(Bool)
                ...
                if Page.Active then
                    ...
                    Items["Inactive"]:ChangeItemTheme({TextColor3 = function()
                        return Library.Theme.Muted
                    end})
                    Items["Inactive"]:Tween(nil, {TextColor3 = Library.Theme.Muted})
                    Items["Accent"]:Tween(nil, {BackgroundTransparency = 1})
                end
        ...
        Library.Sections.Toggle = function(self, Data)
            ...
            function Toggle:Set(Value)
                Toggle.Value = Value 
                Library:SetFlag(Toggle.Flag, Value)
        ...
        Library.Sections.Dropdown = function(self, Data)
            ...
            function Dropdown:Add(Option)
                ...
                function OptionData:Set()
                    ...
                    if Dropdown.Multi then 
                        ...
                        Library:SetFlag(Dropdown.Flag, Dropdown.Value)
                        ...
                        if OptionData.Selected then 
                            ...
                            Library:SetFlag(Dropdown.Flag, OptionData.Name)
                            ...
                            Dropdown.Value = nil
                            Library:SetFlag(Dropdown.Flag, nil)
            ...
            function Dropdown:Set(Option)
                if Dropdown.Multi then 
                    ...
                    Library:SetFlag(Dropdown.Flag, Option)
        ...
        Library.Sections.Textbox = function(self, Data)
            ...
            function Textbox:Set(Value)
                ...
                Library:SetFlag(Textbox.Flag, Value)
    ...
    Library.CreateSettingsPage = function(self, Window, KeybindList, Watermark)
        ...
        local MenuSection = SettingsSubPage:Section({Name = "Settings"})
        ...
        MenuSection:Label("Preferences are saved automatically")
        MenuSection:Button({Name = "Unload", Callback = function()
            Library:Unload()
        end})

        local ThemeSection = SettingsSubPage:Section({Name = "Theming"})
        local AddThemePicker = function(Name, Slot)
            ThemeSection:Label(Name):Colorpicker({
                Flag = "Theme " .. Slot,
                Default = Library.Theme[Slot],
                Alpha = 0,
                Callback = function(Value)
                    Library:ChangeTheme(Slot, Value)
                end
            })
        end

        AddThemePicker("Accent", "Accent")
        AddThemePicker("Accent Shadow", "Dark Accent")
        AddThemePicker("Background", "Background")
        AddThemePicker("Surface", "Surface")
        AddThemePicker("Element", "Element")
        AddThemePicker("Border", "Border")
        AddThemePicker("Text", "Text")
        AddThemePicker("Inactive Text", "Muted")

        return SettingsPage
    end
end

-- Load the single settings file before controls are registered.  Each control
-- consumes its matching pending value when it is created, so order no longer
-- matters and no profile picker/folder is needed.
